...
const getApiBaseUrl = () => {
  ...
  // Si existe una URL configurada en Vercel, tiene prioridad.
  ...
  // Desarrollo desde una red local.
  ...
  // Producción: Vercel (frontend) -> Render (FastAPI).
...
const goToMechanicPanel = () => {
  ...
  const backendUrl = getApiBaseUrl().replace(/\/$/, "");

  window.location.assign(`${backendUrl}/mecanico`);
...
function App() {
  ...
  const handleLogout = (immediate = false) => {
    ...
    // El iframe del administrador ya cerró la sesión del backend. No pedimos

    // una segunda confirmación: regresamos inmediatamente al inicio público.
