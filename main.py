...
def normalizar_rol_empleado(valor) -> Optional[str]:
    """
    Normaliza el rol de un empleado.

    Permite reconocer correctamente textos con o sin tildes,
    por ejemplo: "Mecánico" -> "mecanico" y "Grúa" -> "conductor_grua".
    """
    rol = str(valor or "").strip().lower()

    # Eliminar tildes para que "mecánico" y "mecanico"
    # sean tratados de la misma manera.
    rol = unicodedata.normalize("NFD", rol)
    rol = "".join(
        caracter
        for caracter in rol
    ...
    # Unificar espacios y guiones.
    rol = rol.replace(" ", "_").replace("-", "_")
...
def generar_codigo_documento(conn, table: str, column: str, prefix: str):
    ...
    return f"{base}-{int(total_dia) + 1:04d}"
def generar_firma_integridad_wompi(referencia: str, amount_in_cents: int) -> str:
    texto = f"{referencia}{amount_in_cents}COP{WOMPI_INTEGRITY_SECRET}"
    return hashlib.sha256(texto.encode("utf-8")).hexdigest()

def generar_checkout_wompi_pedido(
    referencia: str,
    amount_in_cents: int,
    email: Optional[str] = None,
) -> str:
    # Genera un Checkout Web de Wompi asociado a un pedido concreto.
    if not WOMPI_PUBLIC_KEY:
        raise RuntimeError("Falta configurar WOMPI_PUBLIC_KEY")
    if not WOMPI_INTEGRITY_SECRET:
        raise RuntimeError("Falta configurar WOMPI_INTEGRITY_SECRET")
    if amount_in_cents <= 0:
        raise ValueError("El monto del pedido debe ser mayor que cero")

    firma = generar_firma_integridad_wompi(
        referencia,
        amount_in_cents,
    )
    ...
    parametros = {
        ...
        "redirect-url": "https://dmi-sistema-de-informacion.vercel.app/",
    }

    if email:
        parametros["customer-data:email"] = email
...
async def actualizar_mi_perfil(
    ...
    if not usuario:
        return JSONResponse(
            {
                "success": False,
                "error": "Tu sesión no es válida o tu cuenta ya no existe.",
            },
            status_code=401,
        )
    ...
    if not usuario_id:
        return JSONResponse(
            {
                "success": False,
                "error": "No encontramos tu perfil de usuario en DMI.",
            },
            status_code=404,
        )
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            if "apellidos" in columnas:
                campos.append("apellidos = :apellidos")
            ...
            if not campos:
                return JSONResponse(
                    {
                        "success": False,
                        "error": "No encontramos campos editables en el perfil.",
                    },
                    status_code=500,
                )
        ...
        if not perfil:
            return JSONResponse(
                {
                    "success": False,
                    "error": "No fue posible recuperar el perfil actualizado.",
                },
                status_code=404,
            )
        ...
        perfil_actualizado = {
            key: value
            for key, value in dict(perfil).items()
        }
        ...
        print("ERROR actualizar_mi_perfil:", e)
...
def validar_fecha_hora_cita(fecha_cita: str, hora_cita: str) -> tuple[date, time]:
    ...
    hoy = datetime.now(ZoneInfo("America/Bogota")).date()
    # Política de citas DMI: máximo dos meses, lunes a sábado, 9 AM a 6 PM.
    ...
    if fecha > limite:
        raise HTTPException(
            status_code=400,
            detail=f"Solo puedes agendar una cita hasta dos meses hacia adelante. La fecha máxima permitida es {limite.strftime('%d/%m/%Y')}."
        )
...
async def crear_mi_vehiculo(
    ...
    cliente_id = usuario.get("idusuarios")
    ...
    try:
        ...
        motor = str(body.get("motor", "")).strip()
        cantidad_asientos = str(
            body.get("cantidad_asientos", "")
        ).strip()
        capacidad = str(
            body.get("capacidad", "")
        ).strip()
        ...
        with engine.connect() as conn:
            ...
            codigo = f"VEH-{int(siguiente_id):03d}"
            ...
            vehiculo = conn.execute(
                text("""
                    SELECT
                        ...
                        v.estado,
                        v.activo,
                        tv.codigotipovehiculos,
                        tv.vehiculo AS tipo_vehiculo
                    FROM dmi.vehiculos v
                    LEFT JOIN dmi.tipovehiculos tv
                        ON tv.idtipovehiculos =
                           v.tipovehiculos_idtipovehiculos
        ...
        print("ERROR POST /api/mis-vehiculos:", e)
    ...
    # ============================================================
# API FLUTTER - MIS VEHICULOS
# ============================================================

@app.get("/api/mis-vehiculos")
async def api_mis_vehiculos(
    request: Request,
    access_token: str = Cookie(None),
):
    """
    Devuelve únicamente los vehículos pertenecientes
    al usuario autenticado.

    Relación:
        dmi.usuarios.idusuarios
                ↓
        dmi.vehiculos.cliente_id
    """
    ...
    try:
        with engine.connect() as conn:
            ...
            return JSONResponse(
                [json_row(row) for row in vehiculos]
            )

    except Exception as e:

        print("ERROR /api/mis-vehiculos:", e)

        return JSONResponse(
            {
                "error": "No fue posible consultar tus vehiculos.",
                "detail": str(e),
            },
            status_code=500,
        )
...
async def registrar_pedido_checkout(request: Request, access_token: str = Cookie(None)):
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            conn.execute(text("ALTER TABLE public.pedidos ADD COLUMN IF NOT EXISTS referencia_wompi varchar(120)"))
            conn.execute(text("ALTER TABLE public.pedidos ADD COLUMN IF NOT EXISTS wompi_transaction_id varchar(120)"))
            conn.execute(text("ALTER TABLE public.pedidos ADD COLUMN IF NOT EXISTS wompi_estado varchar(40)"))
            ...
            referencia_wompi = (
               f"DMI-PEDIDO-{codigo_pedido}-"
               f"{int(datetime.now().timestamp() * 1000)}"
              ) if tipo_pago == "wompi" else None
            valores = {
        ...
        checkout_url = None
        if tipo_pago == "wompi":
            monto_en_centavos = int(round(total_calculado * 100))
...
async def recibir_evento_wompi(request: Request):
    ...
    try:
        ...
        # ============================================================
        # PEDIDO DE REPUESTOS DEL CATALOGO
        # ============================================================
        if len(partes) >= 4 and partes[0] == "DMI" and partes[1] == "PEDIDO":
            monto_recibido = int(transaccion.get("amount_in_cents") or 0)

            if monto_recibido <= 0:
                return JSONResponse(
                    {"error": "Monto de transaccion invalido"},
                    status_code=400,
                )
            ...
            with engine.connect() as conn:
                pedido = conn.execute(
                    text("""
                        SELECT *
                        FROM public.pedidos
                        WHERE referencia_wompi = :referencia
                        LIMIT 1
                    """),
                    {"referencia": referencia},
                ).mappings().fetchone()
                ...
                if not pedido:
                    return JSONResponse({
                        "ok": True,
                        "ignored": True,
                        "reason": "Pedido DMI no encontrado",
                    })

                monto_esperado = int(
                    round(float(pedido.get("total") or 0) * 100)
                )

                if monto_recibido != monto_esperado:
                    return JSONResponse(
                        {
                            "error": "El monto de Wompi no coincide con el pedido",
                        },
                        status_code=400,
                    )

                transaccion_id = str(
                    transaccion.get("id") or referencia
                )
                ...
                conn.execute(
                    text("""
                        ...
                        SET
                            ...
                            wompi_estado = 'APPROVED',
                            estado = 'pedido_aceptado',
                            novedad = 'Pago Wompi aprobado. Pedido aceptado para preparacion.'
                        WHERE referencia_wompi = :referencia
                    """),
                    {
                        "transaction_id": transaccion_id,
        ...
        if len(partes) < 4 or partes[0] != "DMI" or partes[1] != "FACTURA":
            ...
            return JSONResponse({
                "ok": True,
                "ignored": True,
                "reason": "Referencia no asociada a DMI",
            })
